...
public:
    ...
    void displayEndOfDaySummary() const {
        ...
        for (int trip = 0; trip < NUM_TRIPS; ++trip) {
            ...
            totalPassengers += passengers_up[trip] + passengers_down[trip];
            totalMoney += money_up[trip] + money_down[trip];

            if (passengers_up[trip] > maxPassengers) {
                maxPassengers = passengers_up[trip];
                maxPassengersTrip = trip + 1;
            }

            if (passengers_down[trip] > maxPassengers) {
                maxPassengers = passengers_down[trip];
                maxPassengersTrip = trip + 1;
            }
        }

        cout << "\nTotal Passengers for the Day: " << totalPassengers << "\n";
        cout << "Total Money for the Day: $" << totalMoney << "\n";
        cout << "Train Journey with the Most Passengers: Train " << maxPassengersTrip << "\n";
    }
};
...
int main() {
    ...
    railway.displayEndOfDaySummary();
